...
#include <voilk/chain/buffer_type.hpp>
...
using voilk::protocol::chain_id_type;
...
enum object_type
{
   ...
   withdraw_coining_route_object_type,
   ...
   coining_delegation_object_type,
   coining_delegation_expiration_object_type,
...
class withdraw_coining_route_object;
...
class coining_delegation_object;
class coining_delegation_expiration_object;
...
typedef oid< withdraw_coining_route_object          > withdraw_coining_route_id_type;
...
typedef oid< coining_delegation_object              > coining_delegation_id_type;
typedef oid< coining_delegation_expiration_object   > coining_delegation_expiration_id_type;
...
{
   class variant;
   inline void to_variant( const voilk::chain::shared_string& s, variant& var )
   {
      var = fc::string( voilk::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, voilk::chain::shared_string& s )
   {
   ...
   namespace raw {
      ...
      template< typename T >
      inline T unpack_from_vector( const voilk::chain::buffer_type& s )
      { try  {
         T tmp;
         if( s.size() ) {
...
FC_REFLECT_ENUM( voilk::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
#ifndef ENABLE_STD_ALLOCATOR
FC_REFLECT_TYPENAME( voilk::chain::shared_string )
#endif

FC_REFLECT_ENUM( voilk::chain::bandwidth_type, (post)(forum)(market) )
