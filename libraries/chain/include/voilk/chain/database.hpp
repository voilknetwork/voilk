 ...
 */
#pragma once
#include <voilk/chain/block_log.hpp>
#include <voilk/chain/fork_database.hpp>
#include <voilk/chain/global_property_object.hpp>
#include <voilk/chain/hardfork_property_object.hpp>
#include <voilk/chain/node_property_object.hpp>
#include <voilk/chain/notifications.hpp>

#include <voilk/chain/util/advanced_benchmark_dumper.hpp>
#include <voilk/chain/util/signal.hpp>
...
namespace voilk { namespace chain {
   ...
   using voilk::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = VOILK_INIT_SUPPLY;
         ...
         chain_id_type voilk_chain_id = VOILK_CHAIN_ID;
         ...
         */
         ...
         asset get_effective_coining_shares( const account_object& account, asset_symbol_type coined_symbol )const;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns VOILK_NULL_WITNESS
          */
         ...
         /** @return the vsd created and deposited to_account, may return VOILK if there is no median feed */
         std::pair< asset, asset > create_vsd( const account_object& to_account, asset voilk, bool to_reward_balance=false );
         asset create_coining( const account_object& to_account, asset voilk, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& vsd, const asset& curator_vsd_value, const asset& beneficiary_value );
         ...
         void        adjust_reward_balance( const account_name_type& name, const asset& value_delta,
                                            const asset& share_delta = asset(0,COINS_SYMBOL) );
         void        adjust_supply( const asset& delta, bool adjust_coining = false );
         ...
         /** this updates the votes for all witnesses as a result of account COINS changing */
         ...
         void process_coining_withdrawals();
         ...
         /**
          * Helper method to return the current vsd value of a given amount of
          * VOILK.  Return 0 VSD if there isn't a current_median_history
          */
         asset to_vsd( const asset& voilk )const;
         asset to_voilk( const asset& vsd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = VOILK_INIT_SUPPLY );
         ...
         void perform_coining_share_split( uint32_t magnitude );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ VOILK_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ VOILK_NUM_HARDFORKS + 1 ];
