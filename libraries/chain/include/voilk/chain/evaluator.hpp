...
template< typename OperationType=voilk::protocol::operation >
class evaluator
{
   public:
      virtual ~evaluator() {}
...
template< typename EvaluatorType, typename OperationType=voilk::protocol::operation >
...
#define VOILK_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public voilk::chain::evaluator_impl< X ## _evaluator > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( database& db )                                       \
         : voilk::chain::evaluator_impl< X ## _evaluator >( db )          \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
};

#define VOILK_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )               \
class X ## _evaluator : public voilk::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( voilk::chain::database& db, PLUGIN* plugin )       \
         : voilk::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
