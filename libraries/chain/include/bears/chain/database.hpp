 ...
 */
#pragma once
#include <bears/chain/block_log.hpp>
#include <bears/chain/fork_database.hpp>
#include <bears/chain/global_property_object.hpp>
#include <bears/chain/hardfork_property_object.hpp>
#include <bears/chain/node_property_object.hpp>
#include <bears/chain/notifications.hpp>

#include <bears/chain/util/advanced_benchmark_dumper.hpp>
#include <bears/chain/util/signal.hpp>
...
namespace bears { namespace chain {
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = BEARS_INIT_SUPPLY;
         ...
         chain_id_type bears_chain_id = BEARS_CHAIN_ID;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns BEARS_NULL_WITNESS
          */
         ...
         /** @return the bsd created and deposited to_account, may return BEARS if there is no median feed */
         std::pair< asset, asset > create_bsd( const account_object& to_account, asset bears, bool to_reward_balance=false );
         asset create_coining( const account_object& to_account, asset bears, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& bsd, const asset& curator_bsd_value, const asset& beneficiary_value );
         ...
         /**
          * Helper method to return the current bsd value of a given amount of
          * BEARS.  Return 0 BSD if there isn't a current_median_history
          */
         asset to_bsd( const asset& bears )const;
         asset to_bears( const asset& bsd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = BEARS_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ BEARS_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ BEARS_NUM_HARDFORKS + 1 ];
