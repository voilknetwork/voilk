...
/**Note that the object represents both liquid and coining variant of SMT.
 * The same object is returned by indices when searched by liquid/coining symbol/nai.
 */
...
public:
   ...
   price    one_coining_to_one_liquid() const
   {
      ...
      // ^ On the assumption that liquid and coining SMT have the same precision. See issue 2212
   }

   price    get_coining_share_price() const
   {
      if ( total_coining_fund_smt == 0 || total_coining_shares == 0 )
         return one_coining_to_one_liquid();
         // ^ In original method of globa_property_object it was one liquid to one coining which seems to be a bug.

      return price( asset( total_coining_shares, liquid_symbol.get_paired_symbol() ), asset( total_coining_fund_smt, liquid_symbol ) );
   }

   price    get_reward_coining_share_price() const
   {
      share_type reward_coining_shares = total_coining_shares + pending_rewarded_coining_shares;
      share_type reward_coining_smt = total_coining_fund_smt + pending_rewarded_coining_smt;

      if( reward_coining_shares == 0 || reward_coining_smt == 0 )
          return one_coining_to_one_liquid();
      // ^ Additional check not found in original get_reward_coining_share_price. See issue 2212

      return price( asset( reward_coining_shares, liquid_symbol.get_paired_symbol() ), asset( reward_coining_smt, liquid_symbol ) );
   }

   // id_type is actually oid<smt_token_object>
   id_type           id;

   /**The object represents both liquid and coining variant of SMT
    * To get coining symbol, call liquid_symbol.get_paired_symbol()
    */
   ...
   share_type  total_coining_fund_smt = 0;
   share_type  total_coining_shares = 0;
   share_type  pending_rewarded_coining_shares = 0;
   share_type  pending_rewarded_coining_smt = 0;

   smt_market_maker_state  market_maker;

   /// set_setup_parameters
   bool              allow_voting = false;
   bool              allow_coining = false;
   ...
   /// smt_setup_emissions
   time_point_sec       schedule_time = BEARS_GENESIS_TIME;
   bears::protocol::
   smt_emissions_unit   emissions_unit;
   uint32_t             interval_seconds = 0;
   uint32_t             interval_count = 0;
   time_point_sec       lep_time = BEARS_GENESIS_TIME;
   time_point_sec       rep_time = BEARS_GENESIS_TIME;
   asset                lep_abs_amount = asset( 0, BEARS_SYMBOL );
   asset                rep_abs_amount = asset( 0, BEARS_SYMBOL );
   ...
   // smt_cap_reveal
   share_type  bears_units_min_cap = -1;
   share_type  bears_units_hard_cap = -1;
};
 ...
 * for both liquid and coining symbol/nai.
 */
struct coining_liquid_less
{
   bool operator ()( const asset_symbol_type& lhs, const asset_symbol_type& rhs ) const
   {
      ...
      return ( lhs.is_coining() ? lhs.get_paired_symbol() : lhs ) <
             ( rhs.is_coining() ? rhs.get_paired_symbol() : rhs );
   }
...
typedef multi_index_container <
   ...
   indexed_by <
      ...
      ordered_unique< tag< by_symbol >,
         member< smt_token_object, asset_symbol_type, &smt_token_object::liquid_symbol >, coining_liquid_less >,
...
} } // namespace bears::chain

FC_REFLECT_ENUM( bears::chain::smt_phase,
                  (account_elevated)
                  (setup_completed)
                  (contribution_begin_time_completed)
                  (contribution_end_time_completed)
...
FC_REFLECT( bears::chain::smt_token_object::smt_market_maker_state,
   (bears_balance)
   (token_balance)
   (reserve_ratio)
)

FC_REFLECT( bears::chain::smt_token_object,
   (id)
   (liquid_symbol)
   (control_account)
   (phase)
   (current_supply)
   (total_coining_fund_smt)
   (total_coining_shares)
   (pending_rewarded_coining_shares)
   (pending_rewarded_coining_smt)
   (market_maker)
   (allow_voting)
   (allow_coining)
   (schedule_time)
   ...
   (bears_units_min_cap)
   (bears_units_hard_cap)
)

FC_REFLECT( bears::chain::smt_event_token_object,
   (id)
   (parent)
   (phase)
   (generation_begin_time)
...
CHAINBASE_SET_INDEX_TYPE( bears::chain::smt_token_object, bears::chain::smt_token_index )
CHAINBASE_SET_INDEX_TYPE( bears::chain::smt_event_token_object, bears::chain::smt_event_token_index )
