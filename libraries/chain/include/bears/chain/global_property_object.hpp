...
namespace bears { namespace chain {
   ...
   class dynamic_global_property_object : public object< dynamic_global_property_object_type, dynamic_global_property_object>
   {
      public:
         ...
         asset       virtual_supply             = asset( 0, BEARS_SYMBOL );
         asset       current_supply             = asset( 0, BEARS_SYMBOL );
         asset       confidential_supply        = asset( 0, BEARS_SYMBOL ); ///< total asset held in confidential balances
         ...
         asset       total_coining_fund_bears   = asset( 0, BEARS_SYMBOL );
         asset       total_coining_shares       = asset( 0, COINS_SYMBOL );
         asset       total_reward_fund_bears    = asset( 0, BEARS_SYMBOL );
         fc::uint128 total_reward_shares2; ///< the running total of REWARD^2
         asset       pending_rewarded_coining_shares = asset( 0, COINS_SYMBOL );
         asset       pending_rewarded_coining_bears = asset( 0, BEARS_SYMBOL );

         price       get_coining_share_price() const
         {
            if ( total_coining_fund_bears.amount == 0 || total_coining_shares.amount == 0 )
               return price ( asset( 1000, BEARS_SYMBOL ), asset( 1000000, COINS_SYMBOL ) );

            return price( total_coining_shares, total_coining_fund_bears );
         }

         price get_reward_coining_share_price() const
         {
            return price( total_coining_shares + pending_rewarded_coining_shares,
               total_coining_fund_bears + pending_rewarded_coining_bears );
         }
...
FC_REFLECT( bears::chain::dynamic_global_property_object,
             (id)
             (head_block_number)
             (head_block_id)
             (time)
             ...
             (pending_rewarded_coining_bears)
             (bsd_interest_rate)
             (bsd_print_rate)
             (maximum_block_size)
             (current_aslot)
...
CHAINBASE_SET_INDEX_TYPE( bears::chain::dynamic_global_property_object, bears::chain::dynamic_global_property_index )
