...
void database::open( const open_args& args )
{
   try
   {
      ...
      if( account != nullptr && account->to_withdraw < 0 )
      {
         ...
         {
            a.to_withdraw = 0;
            a.next_coining_withdrawal = fc::time_point_sec::maximum();
         });
         session.squash();
      }
   }
...
#pragma message( "TODO: Update the code below when delegation is modified to support SMTs." )
   const account_regular_balance_object* bo = find< account_regular_balance_object, by_owner_liquid_symbol >(
      boost::make_tuple( account.name, coined_symbol.get_paired_symbol() ) );
   if( bo == nullptr )
      return asset( 0, coined_symbol );
...
// Create coining, then a caller-supplied callback after determining how many shares to create, but before
// we modify the database.
// This allows us to implement virtual op pre-notifications in the Before function.
template< typename Before >
asset create_coining2( database& db, const account_object& to_account, asset liquid, bool to_reward_balance, Before&& before_coining_callback )
{
   try
   {
      auto calculate_new_coining = [ liquid ] ( price coining_share_price ) -> asset
          ...
          *
          *  If Cn equals o.amount, then we must solve for Vn to know how many new coining shares
          *  the user should receive.
          *
          *  128 bit math is requred due to multiplying of 64 bit numbers. This is done in asset and price.
          */
         asset new_coining = liquid * ( coining_share_price );
         return new_coining;
         };
      ...
      if( liquid.symbol.space() == asset_symbol_type::smt_nai_space )
      {
         ...
         price coining_share_price = to_reward_balance ? smt.get_reward_coining_share_price() : smt.get_coining_share_price();
         // Calculate new coining from provided liquid using share price.
         asset new_coining = calculate_new_coining( coining_share_price );
         before_coining_callback( new_coining );
         // Add new coining to owner's balance.
         if( to_reward_balance )
            db.adjust_reward_balance( to_account, liquid, new_coining );
         else
            db.adjust_balance( to_account, new_coining );
         // Update global coining pool numbers.
         db.modify( smt, [&]( smt_token_object& smt_object )
         {
            if( to_reward_balance )
            {
               smt_object.pending_rewarded_coining_shares += new_coining.amount;
               smt_object.pending_rewarded_coining_smt += liquid.amount;
            }
            else
            {
               smt_object.total_coining_fund_smt += liquid.amount;
               smt_object.total_coining_shares += new_coining.amount;
            }
         } );

         // NOTE that SMT coining does not impact witness voting.
...
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
      ...
      if( !to_reward_balance )
         db.adjust_proxied_witness_votes( to_account, new_coining.amount );
...
{
   return create_coining2( *this, to_account, liquid, to_reward_balance, []( asset coins_created ) {} );
}
...
void database::process_coining_withdrawals()
{
   const auto& widx = get_index< account_index, by_next_coining_withdrawal >();
   const auto& didx = get_index< withdraw_coining_route_index, by_withdraw_route >();
   auto current = widx.begin();

   const auto& cprops = get_dynamic_global_properties();

   while( current != widx.end() && current->next_coining_withdrawal <= head_block_time() )
   {
      const auto& from_account = *current; ++current;

      /**
      *  Let T = total tokens in coining fund
      *  Let V = total coining shares
      *  Let v = total coining shares being cashed out
      *
      *  The user may withdraw  vT / V tokens
      */
      share_type to_withdraw;
      if ( from_account.to_withdraw - from_account.withdrawn < from_account.coining_withdraw_rate.amount )
         to_withdraw = std::min( from_account.coining_shares.amount, from_account.to_withdraw % from_account.coining_withdraw_rate.amount ).value;
      else
           ...
           ++itr )
      {
         if( itr->auto_coin )
         {
            ...
            if( to_deposit > 0 )
            {
               ...
               operation vop = fill_coining_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, COINS_SYMBOL ), asset( to_deposit, COINS_SYMBOL ) );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.coining_shares.amount += to_deposit;
               });
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.coining_withdraw_rate.amount = 0;
            a.next_coining_withdrawal = fc::time_point_sec::maximum();
         }
         else
         {
 ...
 *
 *  This method pays out coining and reward shares every block, and liquidity shares once per day.
...
void database::process_funds()
{
   ...
   if( has_hardfork( BEARS_HARDFORK_0_16__551) )
   {
      ...
      share_type new_bears = ( int64_t (BEARS_INIT_SUPPLY/BEARS_1_PERCENT) * 2 * current_inflation_rate ) / ( int64_t( BEARS_100_PERCENT ) * int64_t( BEARS_BLOCKS_PER_YEAR ) );
   ...
   else
   {
      ...
      auto coining_reward = content_reward + curate_reward + witness_pay;
...
asset database::get_producer_reward()
{
   ...
   if( props.head_block_number >= BEARS_START_MINER_VOTING_BLOCK || (witness_account.coining_shares.amount.value == 0) )
   {
      ...
      create_coining2( *this, witness_account, pay, false,
         [&]( const asset& coining_shares )
         {
            vop.get< producer_reward_operation >().coining_shares = coining_shares;
            pre_push_virtual_operation( vop );
...
void database::initialize_indexes()
{
   ...
   add_core_index< withdraw_coining_route_index            >(*this);
   ...
   add_core_index< coining_delegation_index                >(*this);
...
{
   auto now = head_block_time();
   const auto& delegations_by_exp = get_index< coining_delegation_expiration_index, by_expiration >();
   auto itr = delegations_by_exp.begin();
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      operation vop = return_coining_delegation_operation( itr->delegator, itr->coining_shares );
      pre_push_virtual_operation( vop );
...
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
         ...
         a.delegated_coining_shares -= itr->coining_shares;
      });
...
{
   asset_symbol_type liquid_symbol = delta.symbol.is_coining() ? delta.symbol.get_paired_symbol() : delta.symbol;
   ...
   else
   {
      ...
      if( is_all_zero )
      {
         ...
         // Remove balance object if both liquid and coining balances are zero.
         remove( *bo );
      }
      else
      {
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         ...
         case BEARS_ASSET_NUM_COINS:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.coining_shares.amount.value >= 0, "Insufficient COINS funds" );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
      ...
      switch( value_delta.symbol.asset_num )
      {
         case BEARS_ASSET_NUM_BEARS:
            ...
            else
            {
               ...
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_coining_balance.amount.value >= 0, "Insufficient reward COINS funds" );
               }
            }
            break;
         case BEARS_ASSET_NUM_BSD:
...
struct smt_regular_balance_operator
{
   ...
   {
      if( is_coining )
         smt_balance.coining += delta;
      else
         smt_balance.liquid += delta;
   }
   int64_t get_combined_balance( const account_regular_balance_object* bo, bool* is_all_zero )
   {
      asset result = is_coining ? bo->coining + delta : bo->liquid + delta;
      *is_all_zero = result.amount.value == 0 && (is_coining ? bo->liquid.amount.value : bo->coining.amount.value) == 0;
...
struct smt_reward_balance_operator
{
   smt_reward_balance_operator( const asset& value_delta, const asset& share_delta )
      : value_delta(value_delta), share_delta(share_delta), is_coining(share_delta.amount.value != 0)
   {
   ...
   void add_to_balance( account_rewards_balance_object& smt_balance )
   {
      if( is_coining )
      {
         smt_balance.pending_coining_value += value_delta;
         smt_balance.pending_coining_shares += share_delta;
      }
      else
   ...
   {
      asset result = is_coining ? bo->pending_coining_value + value_delta : bo->pending_liquid + value_delta;
      *is_all_zero = result.amount.value == 0 &&
                     (is_coining ? bo->pending_liquid.amount.value : bo->pending_coining_value.amount.value) == 0;
      return result.amount.value;
   }
      ...
      default:
      {
         ...
         const account_regular_balance_object* arbo =
            find< account_regular_balance_object, by_owner_liquid_symbol >(
               boost::make_tuple(a.name, symbol.is_coining() ? symbol.get_paired_symbol() : symbol ) );
         if( arbo == nullptr )
         {
         ...
         {
            return symbol.is_coining() ? arbo->coining : arbo->liquid;
         }
#else
      FC_ASSERT( false, "invalid symbol" );
#endif
      ...
      case BEARS_HARDFORK_0_19:
         {
            ...
            vector< const coining_delegation_object* > to_remove;
            const auto& delegation_idx = get_index< coining_delegation_index, by_id >();
            auto delegation_itr = delegation_idx.begin();

            while( delegation_itr != delegation_idx.end() )
            {
               if( delegation_itr->coining_shares.amount == 0 )
                  to_remove.push_back( &(*delegation_itr) );

               ++delegation_itr;
            }

            for( const coining_delegation_object* delegation_ptr: to_remove )
            {
...
void database::validate_invariants()const
{
   try
   {
      ...
      for( auto itr = witness_idx.begin(); itr != witness_idx.end(); ++itr )
         FC_ASSERT( itr->votes <= gpo.total_coining_shares.amount, "", ("itr",*itr) );
...
void database::validate_smt_invariants()const
{
   try
   {
      ...
      typedef struct {
         ...
         asset pending_coining_shares;
         asset pending_coining_value;
      } TCombinedBalance;
      typedef std::map< asset_symbol_type, TCombinedBalance > TCombinedSupplyMap;
      TCombinedSupplyMap theMap;

      // - Process regular balances, collecting SMT counterparts of 'balance' & 'coining_shares'.
      ...
      add_from_balance_index( balance_idx, [ &theMap ] ( const account_regular_balance_object& regular )
      {
         ...
         asset zero_coining = asset( 0, regular.coining.symbol );
         auto insertInfo = theMap.emplace( regular.liquid.symbol,
            TCombinedBalance( { regular.liquid, regular.coining, zero_liquid, zero_coining, zero_liquid } ) );
         if( insertInfo.second == false )
            ...
            existing_balance.coining += regular.coining;
            }
      });
      ...
      {
         asset zero_liquid = asset( 0, rewards.pending_coining_value.symbol );
         asset zero_coining = asset( 0, rewards.pending_coining_shares.symbol );
         auto insertInfo = theMap.emplace( rewards.pending_liquid.symbol, TCombinedBalance( { zero_liquid, zero_coining,
            rewards.pending_liquid, rewards.pending_coining_shares, rewards.pending_coining_value } ) );
         if( insertInfo.second == false )
            {
            TCombinedBalance& existing_balance = insertInfo.first->second;
            existing_balance.pending_liquid += rewards.pending_liquid;
            existing_balance.pending_coining_shares += rewards.pending_coining_shares;
            existing_balance.pending_coining_value += rewards.pending_coining_value;
            }
      });
      ...
      for( auto itr = limit_order_idx.begin(); itr != limit_order_idx.end(); ++itr )
      {
         if( itr->sell_price.base.symbol.space() == asset_symbol_type::smt_nai_space )
         {
            ...
            FC_ASSERT( a.symbol.is_coining() == false );
            asset zero_liquid = asset( 0, a.symbol );
            asset zero_coining = asset( 0, a.symbol.get_paired_symbol() );
            auto insertInfo = theMap.emplace( a.symbol, TCombinedBalance( { a, zero_coining, zero_liquid, zero_coining, zero_liquid } ) );
            if( insertInfo.second == false )
...
#pragma message( "TODO: Add reward_fund_object iteration here once they support SMTs." )
      ...
      for( ; itr != end; ++itr )
      {
         ...
         asset_symbol_type coining_symbol = smt.liquid_symbol.get_paired_symbol();
...
#pragma message( "TODO: Supplement ^ once SMT rewards are implemented" )
         ...
         // Check coining SMT supply.
         asset total_coining_supply = totalIt == theMap.end() ? asset(0, coining_symbol) :
            ( totalIt->second.coining + totalIt->second.pending_coining_shares );
         asset smt_coining_supply = asset(smt.total_coining_shares + smt.pending_rewarded_coining_shares, coining_symbol);
         FC_ASSERT( smt_coining_supply == total_coining_supply,
                    "", ("smt coining supply",smt_coining_supply)("total_coining_supply",total_coining_supply) );
         // Check pending_coining_value
         asset pending_coining_value = totalIt == theMap.end() ? asset(0, smt.liquid_symbol) : totalIt->second.pending_coining_value;
         FC_ASSERT( asset(smt.pending_rewarded_coining_smt, smt.liquid_symbol) == pending_coining_value, "",
            ("smt pending_rewarded_coining_smt", smt.pending_rewarded_coining_smt)("pending_coining_value", pending_coining_value));
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (head_block_num()) );
}
...
void database::perform_coining_share_split( uint32_t magnitude )
{
   try
   {
      modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& d )
      {
         d.total_coining_shares.amount *= magnitude;
         d.total_reward_shares2 = 0;
      } );

      // Need to update all COINS in accounts and the total COINS in the dgpo
...
// 5. The least significant bit is used as liquid/coining variant indicator so the 10 and 90 milions are numbers
//    of liquid/coining *pairs* of reserved/available NAIs.
...
vector< asset_symbol_type > database::get_smt_next_identifier()
{
   ...
   FC_ASSERT( (_next_available_nai & 0x1) == 0, "Can't start with coining version of NAI." );
   uint32_t new_nai = _next_available_nai;
   // Skip coining version of produced NAI - it differs only by least significant bit set.
