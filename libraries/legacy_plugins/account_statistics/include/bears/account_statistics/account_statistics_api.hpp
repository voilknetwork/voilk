#pragma once

#include <bears/account_statistics/account_statistics_plugin.hpp>

#include <fc/api.hpp>

namespace bears { namespace app {
   struct api_context;
} }

namespace bears { namespace account_statistics {

namespace detail
{
   class account_statistics_api_impl;
}

class account_statistics_api
{
   public:
      account_statistics_api( const bears::app::api_context& ctx );

      void on_api_startup();

   private:
      std::shared_ptr< detail::account_statistics_api_impl > _my;
};

} } // bears::account_statistics

FC_API( bears::account_statistics::account_statistics_api, )