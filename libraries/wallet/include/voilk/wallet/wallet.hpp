...
{
   public:
      wallet_api( const wallet_data& initial_data, const voilk::protocol::chain_id_type& _voilk_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of VOILK and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param voilk_fee The amount of the fee to be paid with VOILK
      ...
      /**
       * Transfer funds from one account to another. VOILK and VSD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 VOILK"
       ...
       * @param vsd_amount The amount of VSD to transfer
       * @param voilk_amount The amount of VOILK to transfer
       ...
       * @param escrow_id A unique id for the escrow transfer
       * @param vsd_amount The amount of VSD that will be released
       * @param voilk_amount The amount of VOILK that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer VOILK into a coining fund represented by coining shares (COINS). COINS are required to coining
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * COINS are protected against dilution up until 90% of VOILK is coining.
       *
       * @param from The account the VOILK is coming from
       * @param to The account getting the COINS
       * @param amount The amount of VOILK to coin i.e. "100.00 VOILK"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *    withdrawn and deposited back as VOILK. i.e. "10.000000 COINS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the COINS are withdrawn from.
       * @param to   The account receiving either COINS or VOILK.
      ...
      /**
       *  This method will convert VSD to VOILK at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its VSD i.e. "1.000 VSD"
       *  @param amount The amount of VSD to convert
       *  @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction convert_vsd(
         string from,
         condenser_api::legacy_asset amount,
         bool broadcast = false );

      /**
       * A witness can public a price feed for the VOILK:VSD market. The median price feed is used
       * to process conversion requests from VSD to VOILK.
       *
       ...
       *                       operations defined in `voilk/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for VOILK:VSD
       *
       ...
       *  @param amount_to_sell The amount of either VSD or VOILK you wish to sell
      ...
      /**
       * Vote on a comment to be paid VOILK
       *
...
FC_REFLECT( voilk::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( voilk::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( voilk::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( voilk::wallet::authority_type, (owner)(active)(posting) )

FC_API( voilk::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( voilk::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
