...
{
   public:
      wallet_api( const wallet_data& initial_data, const bears::protocol::chain_id_type& _bears_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
      ...
      /**
       * Returns coining withdraw routes for an account.
       *
       * @param account Account to query routes
       * @param type Withdraw type type [incoming, outgoing, all]
       */
      vector< database_api::api_withdraw_coining_route_object > get_withdraw_routes( string account, condenser_api::withdraw_route_type type = condenser_api::all )const;
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of BEARS and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param bears_fee The amount of the fee to be paid with BEARS
       *  @param delegated_coins The amount of the fee to be paid with delegation
       *  @param new_account_name The name of the new account
       *  @param json_meta JSON Metadata associated with the new account
       *  @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * This method delegates COINS from one account to another.
       *
       * @param delegator The name of the account delegating COINS
       * @param delegatee The name of the account receiving COINS
       * @param coining_shares The amount of COINS to delegate
       * @param broadcast true if you wish to broadcast the transaction
       */
       condenser_api::legacy_signed_transaction delegate_coining_shares(
          string delegator,
          string delegatee,
       ...
       * vote. Vote strength is determined by the accounts coining shares.
       *
      ...
      /**
       * Transfer funds from one account to another. BEARS and BSD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 BEARS"
      ...
      /**
       * Transfer funds from one account to another using escrow. BEARS and BSD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param agent The account acting as the agent in case of dispute
       * @param escrow_id A unique id for the escrow transfer. (from, escrow_id) must be a unique pair
       * @param bsd_amount The amount of BSD to transfer
       * @param bears_amount The amount of BEARS to transfer
       ...
       * @param escrow_id A unique id for the escrow transfer
       * @param bsd_amount The amount of BSD that will be released
       * @param bears_amount The amount of BEARS that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer BEARS into a coining fund represented by coining shares (COINS). COINS are required to coining
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * COINS are protected against dilution up until 90% of BEARS is coining.
       *
       * @param from The account the BEARS is coming from
       * @param to The account getting the COINS
       * @param amount The amount of BEARS to coin i.e. "100.00 BEARS"
       * @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction transfer_to_coining(
         string from,
         string to,
         condenser_api::legacy_asset amount,
         bool broadcast = false);
      ...
      /**
       * Set up a coining withdraw request. The request is fulfilled once a week over the next two year (104 weeks).
       *
       * @param from The account the COINS are withdrawn from
       * @param coining_shares The amount of COINS to withdraw over the next two years. Each week (amount/104) shares are
       *    withdrawn and deposited back as BEARS. i.e. "10.000000 COINS"
       * @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction withdraw_coining(
         string from,
         condenser_api::legacy_asset coining_shares,
         bool broadcast = false );

      /**
       * Set up a coining withdraw route. When coining shares are withdrawn, they will be routed to these accounts
       * based on the specified weights.
       *
       * @param from The account the COINS are withdrawn from.
       * @param to   The account receiving either COINS or BEARS.
       ...
       * @param auto_coin Set to true if the from account should receive the COINS as COINS, or false if it should receive
       *    them as BEARS.
       * @param broadcast true if you wish to broadcast the transaction.
       */
      condenser_api::legacy_signed_transaction set_withdraw_coining_route(
         string from,
         string to,
         uint16_t percent,
         bool auto_coin,
      ...
      /**
       *  This method will convert BSD to BEARS at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its BSD i.e. "1.000 BSD"
       *  @param amount The amount of BSD to convert
       *  @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction convert_bsd(
         string from,
         condenser_api::legacy_asset amount,
         bool broadcast = false );

      /**
       * A witness can public a price feed for the BEARS:BSD market. The median price feed is used
       * to process conversion requests from BSD to BEARS.
       *
       ...
       *                       operations defined in `bears/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for BEARS:BSD
       *
       ...
       *  @param amount_to_sell The amount of either BSD or BEARS you wish to sell
      ...
      /**
       * Vote on a comment to be paid BEARS
       *
...
FC_REFLECT( bears::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( bears::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( bears::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( bears::wallet::authority_type, (owner)(active)(posting) )

FC_API( bears::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( bears::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
