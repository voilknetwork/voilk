...
struct rc_account_api_object
{
   ...
   //
   // asset                 last_coining_shares = asset( 0, COINS_SYMBOL );
};
...
FC_REFLECT( voilk::plugins::rc::get_resource_params_return,
   (resource_names)
   (resource_params)
   (size_info)
   )

FC_REFLECT( voilk::plugins::rc::resource_pool_api_object,
   (pool)
   )

FC_REFLECT( voilk::plugins::rc::get_resource_pool_return,
   (resource_pool)
   )

FC_REFLECT( voilk::plugins::rc::rc_account_api_object,
   (account)
   (rc_manabar)
...
FC_REFLECT( voilk::plugins::rc::find_rc_accounts_args,
   (accounts)
   )

FC_REFLECT( voilk::plugins::rc::find_rc_accounts_return,
   (rc_accounts)
   )
