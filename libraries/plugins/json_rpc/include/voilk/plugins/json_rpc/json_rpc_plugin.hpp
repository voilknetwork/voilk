...
#define VOILK_JSON_RPC_PLUGIN_NAME "json_rpc"

#define JSON_RPC_REGISTER_API( API_NAME )                                                       \
{                                                                                               \
   voilk::plugins::json_rpc::detail::register_api_method_visitor vtor( API_NAME );              \
   for_each_api( vtor );                                                                        \
}
...
#define JSON_RPC_NO_PARAMS          (-33331)
#define JSON_RPC_PARSE_PARAMS_ERROR (-32002)
#define JSON_RPC_ERROR_DURING_CALL  (-32003)

namespace voilk { namespace plugins { namespace json_rpc {
...
class json_rpc_plugin : public appbase::plugin< json_rpc_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = VOILK_JSON_RPC_PLUGIN_NAME; return name; }
   ...
   class register_api_method_visitor
   {
      public:
         register_api_method_visitor( const std::string& api_name )
            : _api_name( api_name ),
              _json_rpc_plugin( appbase::app().get_plugin< voilk::plugins::json_rpc::json_rpc_plugin >() )
         {}
      ...
      private:
         ...
         voilk::plugins::json_rpc::json_rpc_plugin& _json_rpc_plugin;
   };

}

} } } // voilk::plugins::json_rpc

FC_REFLECT( voilk::plugins::json_rpc::api_method_signature, (args)(ret) )
