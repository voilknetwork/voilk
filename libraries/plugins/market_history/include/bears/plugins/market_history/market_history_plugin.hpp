...
// time.
//
#ifndef BEARS_MARKET_HISTORY_SPACE_ID
#define BEARS_MARKET_HISTORY_SPACE_ID 7
#endif

#ifndef BEARS_MARKET_HISTORY_PLUGIN_NAME
#define BEARS_MARKET_HISTORY_PLUGIN_NAME "market_history"
#endif
...
{
   bucket_object_type        = ( BEARS_MARKET_HISTORY_SPACE_ID << 8 ),
   order_history_object_type = ( BEARS_MARKET_HISTORY_SPACE_ID << 8 ) + 1
};
...
class market_history_plugin : public plugin< market_history_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = BEARS_MARKET_HISTORY_PLUGIN_NAME; return name; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   bucket_object_details bears;
   bucket_object_details non_bears;

#ifdef BEARS_ENABLE_SMT
   asset_symbol_type symbol = BSD_SYMBOL;

   price high()const { return asset( non_bears.high, symbol ) / asset( bears.high, BEARS_SYMBOL ); }
   price low()const { return asset( non_bears.low, symbol ) / asset( bears.low, BEARS_SYMBOL ); }
#else
   price high()const { return asset( non_bears.high, BSD_SYMBOL ) / asset( bears.high, BEARS_SYMBOL ); }
   price low()const { return asset( non_bears.low, BSD_SYMBOL ) / asset( bears.low, BEARS_SYMBOL ); }
#endif
};
...
FC_REFLECT( bears::plugins::market_history::bucket_object_details,
            (high)
            (low)
            (open)
            (close)
...
#if defined BEARS_ENABLE_SMT
FC_REFLECT( bears::plugins::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (bears)(symbol)(non_bears)
         )
#else
FC_REFLECT( bears::plugins::market_history::bucket_object,
                     (id)
...
CHAINBASE_SET_INDEX_TYPE( bears::plugins::market_history::bucket_object, bears::plugins::market_history::bucket_index )

FC_REFLECT( bears::plugins::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( bears::plugins::market_history::order_history_object, bears::plugins::market_history::order_history_index )
