...
#include <voilk/plugins/block_data_export/exportable_block_data.hpp>

#include <voilk/plugins/stats_export/stats_export_plugin.hpp>
...
using voilk::chain::block_notification;
using voilk::chain::database;
using voilk::chain::dynamic_global_property_object;
...
using voilk::plugins::block_data_export::exportable_block_data;

using voilk::plugins::chain::chain_plugin;
...
FC_REFLECT( voilk::plugins::stats_export::detail::api_stats_transaction_data_object, (user)(size) )
FC_REFLECT( voilk::plugins::stats_export::detail::api_stats_export_data_object, (global_properties)(transaction_stats)(free_memory) )

namespace voilk { namespace plugins { namespace stats_export { namespace detail {
...
{
   std::shared_ptr< api_stats_export_data_object > stats = _export_plugin.find_export_data< api_stats_export_data_object >( VOILK_STATS_EXPORT_PLUGIN_NAME );
   if( !stats )
      return;
...
void stats_export_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      my->_export_plugin.register_export_data_factory( VOILK_STATS_EXPORT_PLUGIN_NAME,
