...
struct count_operation_visitor
{
   ...
   void operator()( const account_create_with_delegation_operation& op )const
   {
      state_bytes_count +=
         ...
         + get_authority_byte_count( op.active )
         + get_authority_byte_count( op.posting )
         + _w.coining_delegation_object_base_size;
   ...
   void operator()( const delegate_coining_shares_operation& op )const
   {
      state_bytes_count += std::max(
         _w.coining_delegation_object_base_size,
         _w.coining_delegation_expiration_object_base_size
         );
      execution_time_count += _e.delegate_coining_shares_operation_exec_time;
   }
   ...
   void operator()( const set_withdraw_coining_route_operation& op )const
   {
      ...
      state_bytes_count += _w.withdraw_coining_route_object_base_size;
      execution_time_count += _e.set_withdraw_coining_route_operation_exec_time;
   }
   ...
   {
      FC_TODO( "Change RC state bytes computation to take SMT's into account" )
      execution_time_count += _e.transfer_to_coining_operation_exec_time;
      market_op_count++;
   }
   ...
   {
      FC_TODO( "Change RC state bytes computation to take SMT's into account" )
      execution_time_count += _e.withdraw_coining_operation_exec_time;
   }
   ...
   void operator()( const fill_coining_withdraw_operation& ) const {}
   ...
   void operator()( const return_coining_delegation_operation& ) const {}
   ...
   // withdraw_coining, convert, set_withdraw_coining_route, limit_order_create2
   ...
   // claim_reward_balance, delegate_coining_shares, any SMT operations
};
