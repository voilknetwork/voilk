...
"""
  Usage: script_name jobs url1 url2 [wdir [last_block [first_block]]]
    Example: script_name 4 http://127.0.0.1:6990 http://127.0.0.1:6991 ./ 5000000 0
...
def main():
  if len(sys.argv) < 4 or len(sys.argv) > 7:
    ...
    print("  Example: script_name 4 http://127.0.0.1:6990 http://127.0.0.1:6991 ./ 5000000 0")
...
def get_last_block(url, max_tries=10, timeout=0.1):
  ...
  status, response = voilkd_call(url, data=request, max_tries=max_tries, timeout=timeout)
...
def compare_results(f_block, l_block, url1, url2, max_tries=10, timeout=0.1):
  ...
  for i in range(f_block, l_block+1):
    ...
    #with ProcessPoolExecutor(max_workers=2) as executor:
      future1 = executor.submit(voilkd_call, url1, data=request, max_tries=max_tries, timeout=timeout)
      future2 = executor.submit(voilkd_call, url2, data=request, max_tries=max_tries, timeout=timeout)

    status1, json1 = future1.result()
    status2, json2 = future2.result()
    
    #status1, json1 = voilkd_call(url1, data=request, max_tries=max_tries, timeout=timeout)
    #status2, json2 = voilkd_call(url2, data=request, max_tries=max_tries, timeout=timeout)
