...
class DebugNode( object ):
   """ Wraps the voilkd debug node plugin for easier automated testing of the Voilk Network"""

   def __init__( self, voilkd, data_dir, args='', plugins=[], apis=[], voilkd_out=None, voilkd_err=None ):
      """ Creates a voilkd debug node.
      ...
      For all other requests, the python-voilk library should be used.

      args:
         voilkd -- The string path to the location of the voilkd binary
         data_dir -- The string path to an existing voilkd data directory which will be used to pull blocks from.
         args -- Other string args to pass to voilkd.
         ...
         voilkd_stdout -- A stream for voilkd's stdout. Default is to pipe to /dev/null
         voilkd_stderr -- A stream for voilkd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._voilkd_bin = None
      self._voilkd_lock = None
      self._voilkd_process = None
      self._temp_data_dir = None

      self._voilkd_bin = Path( voilkd )
      if( not self._voilkd_bin.exists() ):
         raise ValueError( 'voilkd does not exist' )
      if( not self._voilkd_bin.is_file() ):
         raise ValueError( 'voilkd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed voilk data directory' )
      ...
      if( voilkd_out != None ):
         self.voilkd_out = voilkd_out
      else:
         self.voilkd_out = self._FNULL

      if( voilkd_err != None ):
         self.voilkd_err = voilkd_err
      else:
         self.voilkd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._voilkd_lock = Lock()


   def __enter__( self ):
      self._voilkd_lock.acquire()
      ...
      voilkd = [ str( self._voilkd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      voilkd.extend( self._args )

      self._voilkd_process = Popen( voilkd, stdout=self.voilkd_out, stderr=self.voilkd_err )
      self._voilkd_process.poll()
      sleep( 5 )
      if( not self._voilkd_process.returncode ):
         self._rpc = VoilkNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "voilkd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._voilkd_process != None ):
         self._voilkd_process.poll()

         if( not self._voilkd_process.returncode ):
            self._voilkd_process.send_signal( SIGINT )

            sleep( 7 )
            self._voilkd_process.poll()

            if( not self._voilkd_process.returncode ):
               self._voilkd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._voilkd_process.poll()

               if( self._voilkd_process.returncode ):
                  loggin.error( 'voilkd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._voilkd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._voilkd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key voilk debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is VOILK_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both voilkd and the data directory,
      ...
      debug_node = DebugNode( str( voilkd ), str( data_dir ), voilkd_err=sys.stderr )
