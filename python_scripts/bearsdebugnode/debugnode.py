...
class DebugNode( object ):
   ...
   def __init__( self, bearsd, data_dir, args='', plugins=[], apis=[], bearsd_out=None, bearsd_err=None ):
      ...
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed bears data directory' )
   ...
   def __enter__( self ):
      ...
      bearsd = [ str( self._bearsd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      bearsd.extend( self._args )

      self._bearsd_process = Popen( bearsd, stdout=self.bearsd_out, stderr=self.bearsd_err )
         ...
         if( not self._bearsd_process.returncode ):
            ...
            if( not self._bearsd_process.returncode ):
               ...
               if( self._bearsd_process.returncode ):
                  loggin.error( 'bearsd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both bearsd and the data directory,
      ...
      debug_node = DebugNode( str( bearsd ), str( data_dir ), bearsd_err=sys.stderr )
