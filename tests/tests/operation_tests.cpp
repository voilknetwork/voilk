...
{
   return (uint16_t)( a.voting_manabar.current_mana / chain::util::get_effective_coining_shares( a ) );
}
...
BOOST_AUTO_TEST_CASE( withdraw_coining_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test withdrawing minimal COINS" );
      op.account = "bob";
      op.coining_shares = db->get_account( "bob" ).coining_shares;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
...
BOOST_AUTO_TEST_CASE( delegate_coining_shares_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating delgated COINS." );
