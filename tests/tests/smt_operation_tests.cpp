...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      // Legacy symbols
      asset partial_coins = ASSET( "5.000000 COINS" );
      op.reward_tokens.push_back( ASSET( "0.000 TBD" ) );
      op.reward_tokens.push_back( ASSET( "0.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( smt_transfer_to_coining_validate )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: transfer_to_coining_validate with SMT" );
...
// Here would be smt_transfer_to_coining_authorities if it differed from transfer_to_coining_authorities

BOOST_AUTO_TEST_CASE( smt_transfer_to_coining_apply )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_transfer_to_coining_apply" );

      auto test_single_smt = [this] (const asset_symbol_type& liquid_smt, const fc::ecc::private_key& key )
      {
         asset_symbol_type coining_smt = liquid_smt.get_paired_symbol();
         // Fund creator with SMTs
         FUND( "alice", asset( 10000, liquid_smt ) );

         const auto& smt_object = db->get< smt_token_object, by_symbol >( liquid_smt );

         // Check pre-coining balances
         FC_ASSERT( db->get_balance( "alice", liquid_smt ).amount == 10000, "SMT balance adjusting error" );
         FC_ASSERT( db->get_balance( "alice", coining_smt ).amount == 0, "SMT balance adjusting error" );

         auto smt_shares = asset( smt_object.total_coining_shares, coining_smt );
         auto smt_coins = asset( smt_object.total_coining_fund_smt, liquid_smt );
         auto smt_share_price = smt_object.get_coining_share_price();
         auto alice_smt_shares = db->get_balance( "alice", coining_smt );
         auto bob_smt_shares = db->get_balance( "bob", coining_smt );;
         ...
         auto new_coin = op.amount * smt_share_price;
         smt_shares += new_coin;
         smt_coins += op.amount;
         alice_smt_shares += new_coin;

         BOOST_REQUIRE( db->get_balance( "alice", liquid_smt ) == asset( 2500, liquid_smt ) );
         BOOST_REQUIRE( db->get_balance( "alice", coining_smt ) == alice_smt_shares );
         BOOST_REQUIRE( smt_object.total_coining_fund_smt.value == smt_coins.amount.value );
         BOOST_REQUIRE( smt_object.total_coining_shares.value == smt_shares.amount.value );
         validate_database();
         smt_share_price = smt_object.get_coining_share_price();

         // Transfer Alice's liquid to Bob's coins
         op.to = "bob";
         op.amount = asset( 2000, liquid_smt );
         PUSH_OP( op, key );

         new_coin = op.amount * smt_share_price;
         smt_shares += new_coin;
         smt_coins += op.amount;
         bob_smt_shares += new_coin;
         ...
         BOOST_REQUIRE( db->get_balance( "bob", coining_smt ) == bob_smt_shares );
         BOOST_REQUIRE( smt_object.total_coining_fund_smt.value == smt_coins.amount.value );
         BOOST_REQUIRE( smt_object.total_coining_shares.value == smt_shares.amount.value );
         validate_database();
      };
