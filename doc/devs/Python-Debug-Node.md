...
of a running Voilk Debug Node. The Debug Node is a plugin for Voilk that allows realtime
...
[Python Voilk library](https://github.com/xeroc/python-voilklib).
...
To use the script include `from voilkdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/voilknetwork/voilk/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/voilknetwork/voilk/python_scripts/voilkdebugnode/debugnode.py#L212)
...
specify the voilkd binary location and a populated data directory. For most configurations
this will be `programs/voilkd/voilkd` and `witness_node_data_dir` respectively, from the
git root directory for Voilk.
