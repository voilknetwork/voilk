...
of a running Bears Debug Node. The Debug Node is a plugin for Bears that allows realtime
...
[Python Bears library](https://github.com/xeroc/python-bearslib).
...
To use the script include `from bearsdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/bearshares/bears/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/bearshares/bears/python_scripts/bearsdebugnode/debugnode.py#L212)
...
specify the bearsd binary location and a populated data directory. For most configurations
this will be `programs/bearsd/bearsd` and `witness_node_data_dir` respectively, from the
git root directory for Bears.
