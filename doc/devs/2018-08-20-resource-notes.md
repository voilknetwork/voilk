...
One way to think about decay is "resources expire."  Bears had relatively little activity over its
...
- The maximum RC an account can have is equal to its Bears Power (technically, COINS).
...
of the day), and weekly variations (people use Bears more/less on weekends/holidays).  The default decay rate (347321) corresponds to a half-life of
...
100% of the Bears Power stake is constantly using all of its RC to do transactions that only require a single resource, the usage will be
...
less, than the sum of each individual user's decisions.  The budget creates market forces that influence these decisions:  You need X Bears Power
...
- A value of 10,000 (`BEARS_ACCOUNT_SUBSIDY_PRECISION`) represents a budget of one subsidized account per block.
- `account_subsidy_decay` is the per-block decay rate.  A value of `2^36` (`BEARS_RD_DECAY_DENOM_SHIFT`) represents 100% decay rate.
...
BEARS_BLOCKS_PER_DAY = 20*60*24
BEARS_RD_DECAY_DENOM_SHIFT = 36
BEARS_BLOCKS_PER_DAY = 20*60*24
BEARS_MAX_WITNESSES = 21
BEARS_MAX_VOTED_WITNESSES_HF17 = 20

f = lambda d : int(0.5 + (1 << BEARS_RD_DECAY_DENOM_SHIFT) * (-math.expm1(-math.log(2.0) / (BEARS_BLOCKS_PER_DAY * d * BEARS_MAX_VOTED_WITNESSES_HF17 / BEARS_MAX_WITNESSES))))
...
- Upgrading consensus limits is a hardfork, all `bearsd` nodes must upgrade.
...
- If a witness ignores the subsidized account limit, people will get new accounts for free, that normally cost BEARS to create.  This is a medium-sized economic problem.
...
For resource limits, having witness-only upgrades outweighs the problem.  Witnesses who have been around a long time know that, in the ancient past, Bears's bandwidth
...
we should never implement such operations in the Bears source code!  Even the worst-case CPU cycles / memory bytes consumed by an attacker spamming the most
