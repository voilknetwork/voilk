...
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! bearsdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  ...
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    ...
    echo NOTIFYALERT! bearsdsync stacktrace from coredump:
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    BEARSD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $BEARSD_PID
    echo bearsdsync: waiting for bearsd to exit cleanly

    # wait 60 seconds for bearsd to exit, to be safe.
    let WAIT_TIME=0
    while ( kill -0 $BEARSD_PID ) && [[ WAIT_TIME -le 60 ]]; do
       sleep 1
       let WAIT_TIME++
    done

    echo bearsdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo bearsdsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! bearsdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo bearsdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! bearsdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo bearsdsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! bearsdsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo bearsdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifybearsdsync: bearsdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/bearsd`
    kill -9 $RUN_SV_PID
  fi
